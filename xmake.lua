...
option("ndk_host")
    set_default("")
    set_showmenu(true)
    set_description("NDK prebuilt host triple (linux-x86_64, darwin-x86_64, darwin-arm64, windows-x86_64). Auto-detected when empty.")
option_end()
...
target("jnilog")
    ...
    before_build(function(target)
        ...
        if not os.isfile(cc) then
            raise("ERROR: NDK toolchain not found at '%s' (host=%s api=%s). Ensure ANDROID_NDK_HOME/ANDROID_NDK points to a valid NDK installation containing aarch64-linux-android%s-clang. Override host with `xmake f --ndk_host=...` or API with `xmake f --api_level=...`.", cc, ndk_host, api, api)
        end
